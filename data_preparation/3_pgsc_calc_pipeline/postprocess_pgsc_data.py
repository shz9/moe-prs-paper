...
def extract_pgsc_pcs(biobank):
    ...
    pc_df = pd.read_csv(
        f"data/pgsc_calc_scores/{biobank}/{biobank}/score/{biobank}_popsimilarity.txt.gz",
        sep="\t",
        usecols=[
            "sampleset",
            "FID",
    ...
    covar_df = pd.read_csv(
        f"data/covariates/{biobank}/covars_{biobank}_pcs.txt",
        sep="\t",
        names=["FID", "IID", "Sex", "Age"],
        usecols=[0, 1, 2, 13],
        dtype={"FID": str, "IID": str},
    )
    ...
    # Sort the columns:
    covar_df = covar_df[
        ["FID", "IID", "Sex"] + [f"PC{i}" for i in range(1, 11)] + ["Age"]
    ]
...
def extract_pgsc_ancestry(biobank):
    ...
    anc_df["Ancestry"] = np.where(
        anc_df["MostSimilarPop_LowConfidence"], "OTH", anc_df["MostSimilarPop"]
    )
    ...
    anc_df[["FID", "IID", "AFR", "AMR", "CSA", "EAS", "EUR", "MID", "Ancestry"]].to_csv(
        f"data/covariates/{biobank}/1kghdp_ancestry_assignments.txt",
        sep="\t",
        index=False,
    )
...
def extract_pgsc_scores(biobank, pgs_table):
    ...
    pgs_df.columns = [
        col.replace("_hmPOS_GRCh37", "").replace("_hmPOS_GRCh38", "")
