...
if __name__ == "__main__":
    ...
    for biobank, ancestries in biobank_group_dict.items():
        for anc in ancestries:
            ...
            plt.title(
                f"Cohort = {anc['Name']} samples in {BIOBANK_NAME_MAP_SHORT[biobank]}"
            )
            ...
            plt.tight_layout()
            plt.savefig(
                f"figures/stratified_prediction_accuracy/{args.phenotype}_{biobank}_{anc['Name']}_{args.metric}.eps",
            )
            plt.close()
