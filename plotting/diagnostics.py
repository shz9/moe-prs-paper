...
def plot_loss_curves(model):
    ...
    for i, expert_id in enumerate(model.expert_cols):
        plt.scatter(
            np.arange(expert_loglik.shape[0]), expert_loglik[:, i], label=expert_id
        )
    ...
    for i, expert_id in enumerate(model.expert_cols):
        plt.scatter(
            np.arange(model_weights.shape[0]), model_weights[:, i], label=expert_id
        )
...
def plot_scatter(
    ...
    model_idx = [
        list(prs_dataset.prs_cols).index(x)
        for x in model_subset
        if x in prs_dataset.prs_cols
    ]
