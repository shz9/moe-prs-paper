...
def generate_model_colors(metrics_df, metric, palette="Set2", n_model_types=4):
    ...
    single_hue_order = (
        metrics_df.loc[metrics_df["Model Category"] == single_model_label]
        .groupby("Model Name")[metric]
        .mean()
        .sort_values(ascending=metric == "MSE")
        .index
    )
    ...
    multiprs_hue_order = (
        metrics_df.loc[metrics_df["Model Category"] == "MultiPRS"]
        .groupby("Model Name")[metric]
        .mean()
        .sort_values(ascending=metric == "MSE")
        .index
    ...
    awm_hue_order = (
        metrics_df.loc[metrics_df["Model Category"] == "AncestryWeightedPRS"]
        .groupby("Model Name")[metric]
        .mean()
        .sort_values(ascending=metric == "MSE")
        .index
    ...
    moe_hue_order = (
        metrics_df.loc[metrics_df["Model Category"] == "MoE"]
        .groupby("Model Name")[metric]
        .mean()
        .sort_values(ascending=metric == "MSE")
        .index
...
if __name__ == "__main__":
    ...
    if args.metrics is None:
        if "Incremental_R2" in eval_df.columns:
            ...
            raise Exception(
                "Could not automatically determine the performance metric to plot. "
